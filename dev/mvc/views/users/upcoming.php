...
if($_SESSION['user']){
    if($_SESSION['user']['rol'] === 2){
        ...
        <section id="miForm" class="d-flex align-items-center justify-content-between">
            ...
            <button id="btn-2" class="shadow-sm  btn  bg-primary text-white fs-6 d-flex justify-content-center align-items-center p-1 border rounded-4 mt-3 h-100  fw-semibold section__btn--size" type="submit">
                Próximas
            </button>
        ...
        for($i = 0; $i < sizeof($lists_items); $i++){
            ...
            <span class="fw-semibold ms-1 mb-2 m-0 p-0 fs-6">' . $item_price . '</span>
                    ...
                    <div class="w-100 ul__li__div--scroll">
                        <p class="p-0 m-0 fs-5 fw-semibold" id="textToStrike"><span class="fs-4 fw-bold ">' .$name_list['list_name']."</span><span class='fs-6 fw-bold text-secondary'> > </span>". $lists_items[$i]['item_name'] . '</p>
                    </div>
            ...
            <!-- <div class="d-flex flex-column p-1 pe-3 h-100 justify-content-between">
                <form method="POST" class="d-flex ">
                    <input type="hidden" name="id_item" value="'.$lists_items[$i]['id_item'].'">';
                    echo'
